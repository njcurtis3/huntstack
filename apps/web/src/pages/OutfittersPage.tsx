...
export function OutfittersPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Find Outfitters</h1>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {mockOutfitters.map((outfitter) => (
          <div key={outfitter.id} className="card hover:shadow-lg transition-shadow">
            {/* Image placeholder */}
            <div className="h-48 bg-gradient-to-br from-forest-200 dark:from-forest-800 to-earth-200 dark:to-earth-800 flex items-center justify-center">
              <span className="text-4xl">🏕️</span>
            </div>

            <div className="p-6">
              <div className="flex items-start justify-between mb-2">
                <h3 className="font-semibold text-gray-900 dark:text-gray-100">{outfitter.name}</h3>
                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">{outfitter.priceRange}</span>
              </div>
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm text-gray-500 dark:text-gray-400">({outfitter.reviews} reviews)</span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {outfitter.huntTypes.map((type) => (
                  <span
                    key={type}
                    className="px-2 py-1 bg-forest-50 dark:bg-forest-950 text-forest-700 dark:text-forest-300 text-xs rounded-full"
                  >
                    {type}
                  </span>
                ))}
